...
using SampleApp.Api.Modules.MyModules.Controllers;
...
namespace SampleApp.Modules.MyModules.Tests.Controllers
{
    public class MyModulesControllerTests
    {
        ...
        public async Task GetMany_ReturnsMatchingItems()
        {
            ...
            _myModulesService.GetMany(Arg.Any<FindManyArgs<MyModuleEntity, MyModuleQueryDto>>()).Returns(Task.FromResult<IEnumerable<MyModuleResultDto>>([expectedResult.ToArray()[1]]));
        ...
        {
            // Arrange
            _myModulesService.GetMany(Arg.Any<FindManyArgs<MyModuleEntity, MyModuleQueryDto>>()).Returns(Task.FromResult<IEnumerable<MyModuleResultDto>>([]));
            var filter = new FindManyArgs<MyModuleEntity, MyModuleQueryDto>()
            {
                Where = new MyModuleQueryDto
                {
        ...
        public async Task GetMeta_ReturnsCorrectMetadata()
        {
            ...
            _myModulesService.GetMeta(Arg.Any<FindManyArgs<MyModuleEntity, MyModuleQueryDto>>()).Returns(Task.FromResult(new MetadataDto() { Count = count }));
            var filter = new FindManyArgs<MyModuleEntity, MyModuleQueryDto>()
            {
                Where = new MyModuleQueryDto
                {
