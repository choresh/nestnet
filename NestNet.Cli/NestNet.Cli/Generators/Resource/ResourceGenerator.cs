...
namespace NestNet.Cli.Generators.Resource
{
    internal static class ResourceGenerator
    {
        ...
        private class ResourceGenerationContext
        {
            ...
            public required string CoreResourcePath { get; set; }
            public required string ApiResourcePath { get; set; }
        }
        ...
        private static ResourceGenerationContext? CreateSilentResourceGenerationContext(InputParams inputParams)
        {
            ...
            string resourcePath = Path.Combine(projectDir, "Resources", inputParams.ResourceName);
            ...
            return new ResourceGenerationContext
            {
                ...
                CoreResourcePath = Path.Combine(projectDir, $"{projectName}.Core", "Resources", inputParams.ResourceName),
                ApiResourcePath = Path.Combine(projectDir, projectName, "Resources", inputParams.ResourceName)
            };
        }
        ...
        private static ResourceGenerationContext? CreateInteractiveResourceGenerationContext()
        {
            ...
            string resourcePath = Path.Combine(projectDir, "Resources", resourceName);
            return new ResourceGenerationContext
            {
                ProjectDir = projectDir,
                ProjectName = projectName,
                ...
                SampleInputDtoName = "SampleInputDto",
                SampleOutputDtoName = "SampleOutputDto",
                CoreResourcePath = Path.Combine(projectDir, $"{projectName}.Core", "Resources", resourceName),
                ApiResourcePath = Path.Combine(projectDir, projectName, "Resources", resourceName)
            };
        }
...
{{
    ...
    {{
        ...
        private static void CreateResourceStructure(ResourceGenerationContext context)
        {
            ...
            {
                Directory.CreateDirectory(Path.Combine(context.CoreResourcePath, folder));
            }
            ...
            foreach (var folder in apiFolders)
            {
                Directory.CreateDirectory(Path.Combine(context.ApiResourcePath, folder));
            }
        }
    }
}