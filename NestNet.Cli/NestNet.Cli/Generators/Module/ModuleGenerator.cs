...
namespace NestNet.Cli.Generators
{
    internal static partial class ModuleGenerator
    {
        ...
        public static void Run(InputParams? inputParams = null)
        {
            ...
            {
                // Silent mode
                context = CreateModuleGenerationContext(inputParams);
            }
            else
            {
                // Interactive mode
            ...
            foreach (var projType in Enum.GetValues<ProjType>())
            {
                if (projType == ProjType.Worker)
                { 
                    continue; // This type not soppurted yet
                }
                AnsiConsole.MarkupLine(Helpers.FormatMessage($"\nModule generation ({projType}) - started\n", "green"));

                ModuleGeneratorBase generator;
                switch (projType)
                {
                    case ProjType.Core:
                ...
                var success = generator.Generate(context);

                if (!success)
                {
                    AnsiConsole.MarkupLine(Helpers.FormatMessage($"\nModule generation ({projType}) - ended, unable to generate the module", "green"));
                    return;
                }

                AnsiConsole.MarkupLine(Helpers.FormatMessage($"\nModule generation ({projType}) - ended successfully\n", "green"));
            }
        }

        private static ModuleGenerationContext CreateModuleGenerationContext(InputParams? inputParams = null)
        {
            string moduleName;
            if (inputParams == null)
            {
