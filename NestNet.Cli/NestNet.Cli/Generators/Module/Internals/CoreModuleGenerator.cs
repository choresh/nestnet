...
namespace NestNet.Cli.Generators
{
    ...
    {
        private class CoreModuleGenerator : ModuleGeneratorBase
        {
            ...
            public override void DoGenerate()
            {
                if (Context.GenerateDbSupport)
                {
                    ...
                    Directory.CreateDirectory(Context.ProjectContext!.ModulePath);
...
";
            ...
            private void CreateEntityFile()
            {
                ...
                string entityPath = Path.Combine(Context.ProjectContext!.ModulePath, "Entities", $"{Context.ModuleName}Entity.cs");
            ...
            {
                string dtoContent = Helpers.GetDtoContent(Context.ProjectContext!.ProjectName, Context.ModuleName, Context.PluralizedModuleName, dtoType, baseClass, properties);
                string dtoPath = Path.Combine(Context.ProjectContext!.ModulePath, "Dtos", $"{Helpers.FormatDtoName(Context.ModuleName, dtoType)}.cs");
            ...
            private void CreateDaoFile()
            {
                ...
                string daoPath = Path.Combine(Context.ProjectContext!.ModulePath, "Daos", $"{Context.ModuleName}Dao.cs");
            ...
            private void CreateServiceFile()
            {
                ...
                string servicePath = Path.Combine(Context.ProjectContext!.ModulePath, "Services", $"{Context.PluralizedModuleName}Service.cs");
            ...
            private void CreateDaoTestFile()
            {
                ...
                string testPath = Path.Combine(Context.ProjectContext!.ModulePath, "Tests", "Daos", $"{Context.PluralizedModuleName}DaoTests.cs");
            ...
            private void CreateServiceTestFile()
            {
                ...
                string testPath = Path.Combine(Context.ProjectContext!.ModulePath, "Tests", "Services", $"{Context.PluralizedModuleName}ServiceTests.cs");
                Directory.CreateDirectory(GetDirectoryName(testPath));
                File.WriteAllText(testPath, testContent);
                AnsiConsole.MarkupLine(Helpers.FormatMessage($"Created: {testPath}", "grey"));
            }
...
{{
    ...
    public class {Context.ModuleName}Entity : EntityBase
    {{
...
{{
    public interface I{Context.ModuleName}Dao: IDao<{Context.EntityName}, {Context.QueryDtoName}>
    {{
        // If you add methods to derived class - expose them here.
    }}

    [Injectable(LifetimeType.Scoped)]
    public class {Context.ModuleName}Dao : DaoBase<{Context.EntityName}, {Context.QueryDtoName}>, I{Context.ModuleName}Dao
    {{
        public {Context.ModuleName}Dao(AppDbContext context)
            : base(context, context.Set<{Context.EntityName}>(), ""{Context.ModuleName}Id"")
        {{
        }}
    ...
    {{
        ...
        {{
            ...
            {{
                ...
                {{
                    {Context.ModuleName}Id = srcEntities[1].{Context.ModuleName}Id
                }}
            }};
...
{{
    ...
    {{
        ...
        {{
            ...
            // Act
            var result = await _service.Update(srcEntities[1].{Context.ModuleName}Id, updateDto, ignoreMissingOrNullFields);
        ...
        {{
            ...
            // Act
            var found = await _service.Delete(srcEntities[1].{Context.ModuleName}Id);
